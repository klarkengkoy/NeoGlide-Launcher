...
class HomeRepository @Inject constructor(
    ...
    suspend fun removeAppFromFolder(folderId: Int, packageName: String) = withContext(Dispatchers.IO) {
        ...
        if (category != null) {
            ...
            appRepository.updateAppCategory(packageName, com.samidevstudio.neoglide.domain.model.AppCategory.fromString(category))
        } else {
    ...
    suspend fun removeFolder(folderId: Int) = withContext(Dispatchers.IO) {
        ...
        if (category != null) {
            folderWithApps.apps.forEach { app ->
                appRepository.updateAppCategory(app.packageName, com.samidevstudio.neoglide.domain.model.AppCategory.fromString(category))
            }
        }
    ...
    suspend fun dissolveFolderIfNeeded(folderId: Int) = withContext(Dispatchers.IO) {
        ...
        if (folderWithApps.apps.isEmpty()) {
            ...
            if (category != null) {
                // Drawer Folder: Move to folder's category
                appRepository.updateAppCategory(lastApp.packageName, com.samidevstudio.neoglide.domain.model.AppCategory.fromString(category))
            } else {
